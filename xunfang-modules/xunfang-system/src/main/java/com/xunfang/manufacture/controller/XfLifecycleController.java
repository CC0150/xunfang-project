...
public class XfLifecycleController extends BaseController
{
    ...
    public AjaxResult lifecycleTemplateList() {
        try {
            ...
            if (template == null) return success(Collections.emptyList());
            ...
            list.add(m);
            return success(list);
        } catch (Exception e) { return error("获取生命周期模板失败: " + e.getMessage()); }
    }
    ...
    public AjaxResult lifeBusiness(@RequestParam String templateId,
        ...
        try { return extractBizOps(templateId); }
        catch (Exception e) { return error("获取业务操作失败: " + e.getMessage()); }
    }
    ...
    public AjaxResult lifeBusinessList(@RequestParam String templateId,
                                       ...
                                       @RequestParam(defaultValue = "") String stateId) {
        try { return extractBizOps(templateId); }
        catch (Exception e) { return error("获取业务操作列表失败: " + e.getMessage()); }
    }
    ...
    public AjaxResult lifeState(@RequestParam String templateId,
        ...
        try {
            ...
            return success(list);
        } catch (Exception e) { return error("获取生命周期状态失败: " + e.getMessage()); }
    }
    ...
    private Map<String, Object> findStartState(JSONObject template) throws JSONException {
        // reserved 前几项是完整 phase 数据（含 data.status）
        JSONArray reserved = template.optJSONArray("reserved");
    ...
    private Map<String, Object> findTargetState(JSONObject template, String bizOpId) throws JSONException {
        JSONArray reserved = template.optJSONArray("reserved");
        ...
        // 2. 在 reserved 中找到 id == targetCellId 的 phase（reserved 前7项是完整phase数据）
        for (int i = 0; i < reserved.length(); i++) {
            JSONObject item = reserved.getJSONObject(i);
            // phase 的 id 字段就是 phaseId
    ...
    private Map<String, Object> stateToMap(JSONObject st) throws JSONException {
        Map<String, Object> m = new LinkedHashMap<>();
    ...
    private JSONObject getTemplate() throws Exception {
        ...
        params.put("characterSet", "UTF8"); params.put("isPresentAll", true);
        params.put("isNeedTotal", false); params.put("publicData", "INCLUDE_PUBLIC_DATA");
        JSONObject filter = new JSONObject();
        JSONArray conditions = new JSONArray();
        addCond(conditions, "latest", "true", "=");
        filter.put("conditions", conditions); filter.put("joiner", "and");
